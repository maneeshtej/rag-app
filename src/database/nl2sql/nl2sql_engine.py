...
class NL2SQLEngine:
    ...
    def _generate_sql(self, *, resolved_object: dict) -> tuple[str, list]:
        ...
        aliases = resolved_object["aliases"]

        def qualify(col):
            table, column = col.split(".")
            alias = aliases.get(table)
            if not alias:
                raise ValueError(f"No alias for table {table}")
        ...
        for left, right in joins:
            ...
            if right_tbl not in joined_tables:
                sql += (
                    f"\nJOIN {right_tbl} {aliases[right_tbl]} "
                    f"ON {qualify(left)} = {qualify(right)}"
                )
        ...
        for f in filters:
            ...
            if confidence == "high" and resolved:
                ...
                if not alias:
                    raise ValueError(f"No alias for table {table}")
                where_clauses.append(f"{alias}.id = %s")
